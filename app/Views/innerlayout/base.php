...
<div id="customNotificationModal" class="custom-modal">
    ...
    <div class="custom-modal-body">
        <?php 
            // Check if the session message is an array or a string
            $message = session()->getTempdata('message');
            if (is_array($message)) {
                // If it's an array, implode with <br> to display each message on a new line
                echo implode('<br>', $message);
            } else {
                // If it's not an array, just display the string as is
                echo $message;
            }
        ?>
    </div>
...
<script>
    ...
    codeContent.innerHTML = `<pre><code>${escapeHtml(outputContent)}</code></pre>`; // Display escaped HTML as text
